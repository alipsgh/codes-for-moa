...
/**
 * <p> McDiarmid Drift Detection Method - Euler Scheme (MDDM_E) </p>
 ...
 * @version $Revision: 7 $
 */
public class MDDM_E extends AbstractChangeDetector {
    ...
    public FloatOption lambdaOption = new FloatOption("lambda", 'l', "The lambda of the exponent part.", 0.01, 0, 1);
    ...
    private double cal_sigma() {
        double sum = 0, bound_sum = 0, r = 1, ratio = Math.pow(Math.E, lambda);
        for (int i = 0; i < win.length; i++){
            sum += r;
            r *= ratio;
        }
    ...
    private double cal_w_mean() {
        double total_sum = 0, win_sum = 0, r = 1, ratio = Math.pow(Math.E, lambda);
        for (int i = 0; i < win.length; i++){
             total_sum += r;
            win_sum += win[i] * r;
            r *= ratio;
